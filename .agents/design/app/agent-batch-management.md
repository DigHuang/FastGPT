...
* **权限受限状态（只读无写/管/删权限）**：
  * 对于当前用户仅有查看权限（`!hasManagePer && !isOwner`）的应用：
    * Checkbox 处于禁用状态（展示禁选图标/禁用样式，鼠标光标为 `not-allowed`）。
    * 鼠标悬浮在该 Checkbox 或卡片右上角时，弹出 Tooltip 提示：**「仅有查看权限，无法批量操作」**。
    * 点击该卡片任何区域均**不可选中**。
* **选中与悬浮行为**：
...
2. **卡片层改造** (`projects/app/src/pageComponents/dashboard/agent/List.tsx`)：
   * 增加 `isBatchMode` 控制：
     ...
     * 卡片右上角渲染 Checkbox：
       * 若当前 item 既无编辑/管理权也无 Owner 权，渲染带禁止状态的 Checkbox，并套上 `MyTooltip` 提示「仅有查看权限，无法批量操作」。
...
* `read_only_no_batch_permission`: 仅有查看权限，无法批量操作
