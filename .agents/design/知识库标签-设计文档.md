...
3. 读出目标 Collection 的当前 tags，在内存中计算覆盖、追加选项或移除后的结果（追加仅对选项类型 array 生效，数字类型等标量只允许覆盖）。
...
### 5.3.1 预设选项删除与文件标签同步清理

在选项类型标签（`tagType === 'array'`）的选项管理中，当用户更新预设选项列表并删除了一个或多个既有选项时（`removedOptions = previousOptions.filter(opt => !nextOptions.has(opt))` 且 `removedOptions.length > 0`）：

1. 定位受影响的 Collection：查询当前 `teamId`、`datasetId` 下，`tags` 中包含该 `tagId` 且 `value` 包含任意被删选项的文档（`tags: { $elemMatch: { tagId, value: { $in: removedOptions } } }`）。
2. 在内存中剔除被删除的选项：
   - 针对匹配的标签项，从 `value` 数组中移除所有存在于 `removedOptions` 中的字符串。
   - 若移除后 `value` 为空数组，则将该标签项从 Collection 的 `tags` 数组中彻底移除。
   - 若移除后 `value` 仍有其余选项，则保留剩余选项值。
   - Collection 上的其他非目标标签保持原样。
3. 批量更新：通过 `writeCollectionTags`（`bulkWrite`）批量将处理后的 `tags` 写回。
4. 事务与一致性：当存在传入的 `ClientSession` 时，标签定义更新与 Collection 标签同步清理在同一事务内执行。
...
- [x] 预设选项删除同步清理：在 `updateOneTag` 中识别被移除的预设选项，批量清理关联 Collection 的标签值；当标签值被清空时彻底移除该标签项。
- [x] 补充与更新单测，覆盖删除选项时 Collection 标签剔除、整项删除及无关标签不受影响场景。
- [x] 运行局部测试、App typecheck、相关 lint 和 `git diff --check` 验证。
