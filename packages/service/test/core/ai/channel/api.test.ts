...
describe('aiproxy channel admin client', () => {
  ...
  it('passes transparent pagination and search parameters directly to aiproxy', async () => {
    ...
    await listGroupChannels('fastgpt:tmb:tmb-a', { page: 2, perPage: 50, search: 'qwen' });
    await listGlobalGroupChannels({ groupId: 'fastgpt:tmb:tmb-b', page: 1, perPage: 20 });
    await listSystemChannels({ page: 3, perPage: 10, search: 'gpt' });

    const urls = axiosMock.mock.calls.map((c) => c[0].url);
    expect(urls[0]).toBe(
      'http://aiproxy.test/api/group/fastgpt%3Atmb%3Atmb-a/channels/?page=2&per_page=50&search=qwen'
    );
    expect(urls[1]).toBe(
      'http://aiproxy.test/api/group_channels/?group=fastgpt%3Atmb%3Atmb-b&page=1&per_page=20'
    );
    expect(urls[2]).toBe('http://aiproxy.test/api/channels/?page=3&per_page=10&search=gpt');
  });
  ...
  it('stops pagination on an empty page in listAllSystemChannels even when total is stale', async () => {
