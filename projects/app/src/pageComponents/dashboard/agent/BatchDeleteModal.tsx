...
const getAppTypeIcon = (type: AppTypeEnum): IconNameType => {
  if (AppFolderTypeList.includes(type)) {
    return 'core/app/sandbox/folderLine';
  }
  if (
  ...
  if (type === AppTypeEnum.simple || type === AppTypeEnum.chatAgent) {
    return 'core/app/type/simple';
  }
  return 'core/app/type/workflow';
};
...
const BatchDeleteModal = ({ apps, onClose, onSuccess }: BatchDeleteModalProps) => {
  ...
  const descriptionText = useMemo(() => {
    if (folderCount > 0 && appCount > 0) {
      return `${t('app:delete_apps_and_folders_confirm_title', {
        appCount,
        folderCount
      })}。${t('app:confirm_delete_apps_tip')}`;
    }
    if (folderCount > 0 && appCount === 0) {
      return folderCount > 1
        ? `${t('app:delete_folders_confirm_title', { count: folderCount })}。${t('app:confirm_delete_folders_tip')}`
        : t('app:confirm_delete_folders_tip');
    }
    return appCount > 1
      ? `${t('app:delete_apps_confirm_title', { count: appCount })}。${t('app:confirm_delete_apps_tip')}`
      : t('app:confirm_delete_apps_tip');
  }, [folderCount, appCount, t]);
      ...
      <VStack align={'stretch'} spacing={4}>
        ...
        {hasFilteredNoPermission && (
          <Flex
            ...
            w={'full'}
          >
            <MyIcon name={'common/info'} w={'14px'} h={'14px'} color={'primary.600'} />
        ...
        {deletableApps.length > 0 && (
          <Box
            ...
            w={'full'}
          >
            <Box fontSize={'14px'} fontWeight={'500'} color={'myGray.900'} lineHeight={'20px'}>
              {t('app:will_delete')}
            </Box>
            <VStack align={'stretch'} spacing={'4px'} mt={'10px'} maxH={'140px'} overflowY={'auto'}>
