...
vi.mock('@fastgpt/service/core/ai/model', () => ({
  getModelHandle: async () => ({
    findModelData: ({ modelId }: { modelId: string }) =>
      cachedModels.find((m) => m.id === modelId || m.modelId === modelId)
  })
}));
...
describe('GET /api/core/ai/channel/modelChannels (channels of one model)', () => {
  ...
  it('returns the own-bucket channels of a visible model (root system model)', async () => {
    mockAuth(true);
    vi.mocked(getMemberModelIds).mockResolvedValue(['m1']);
